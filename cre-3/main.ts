 ...
 *                → Gemini AI: did metric reach target? → outcome + confidence
 *            • EVENT (AI-powered):
 *                Confidential HTTP: fetch news from multiple sources
 *                → Gemini AI: did event happen? Cross-check sources → outcome + confidence
 ...
 *                   Confidential HTTP (News API + Gemini), runInNodeMode + Consensus, EVM Write
 *
...
import { configSchema, CATEGORY_CRYPTO, OUTCOME_YES, type Config } from './src/config'
import { readMarketInfo, readResolutionData, resolveCryptoPrice, submitResolveMarket } from './src/evm'
import { resolveWithAI } from './src/gemini'
...
const decodeSettlementRequestedLog = (log: EVMLog): bigint => {
	try {
		...
		if (decoded.eventName !== 'SettlementRequested') {
			throw new Error(
				`Expected SettlementRequested, but found ${decoded.eventName}. Make sure you are selecting the correct log index.`,
			)
		}
		...
		if (err instanceof Error && err.message.includes('AbiEventSignatureNotFoundError')) {
			throw new Error(
				`Log contains an unknown event signature. This usually happens if you select a 'Transfer' log instead of 'SettlementRequested'. Please check the event index in your simulation.`,
			)
		}
...
const onSettlementRequested = (runtime: Runtime<Config>, log: EVMLog): string => {
	...
	runtime.log(`Market: question="${market.question}" category=${market.category} status=${market.status}`)
	runtime.log(
	...
	if (market.category === CATEGORY_CRYPTO) {
		...
		runtime.log(`Branch: ${label} — calling Gemini AI for resolution`)
